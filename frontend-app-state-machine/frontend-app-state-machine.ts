...
export class FrontendAppStateMachine {
    ...
    (action: OpenTerm, last: McdEditorState) => {
      return transformMcd(last)((state: McdEditorState) => {
        state.termState = tap(new McdEditorTermState())((termState: McdEditorTermState) => {
          ...
          action.term.clozes.forEach((c: Cloze) => {
            var speakMatch = c.speakMatch();
            if (!speakMatch) {
              termState.clozes.push(c.segment);
            } else {
              var split = c.segment.split(":");
              termState.language = split[1];
              termState.speakIt = true;
              termState.trainSpeaking = speakMatch[1] === "train";
  ...
  editTermCloze$ = this.mcdTermActionHandler<EditTermClozes>(EditTermClozes,
    (action: EditTermClozes, newState: McdEditorTermState) => {
      ...
      for (; i < editingClozes.length; ++i) {
        ...
        cloze.schedule.intervalMinutes = 60 * 24;
        cloze.schedule.dueAtMinutes = Date.now() / 1000 / 60 + 60 * 2;
      ...
      for (var i = 0; i < clozes.length; ++i) {
        ...
        if (speakMatch) {
          if (language) {
            ...
            cloze.segment = metaSplits[0] + ":";
          }
        }
      }
    });

  flipTrainSpeak$ = this.mcdTermActionHandler<EditTermFlipTrainSpeaking>(EditTermFlipTrainSpeaking,
    (action: EditTermFlipTrainSpeaking, newState: McdEditorTermState) => {
      var newTerm = newState.editing = shallowCopy(newState.editing);
      var trainSpeaking = newState.trainSpeaking = !newState.trainSpeaking;
      newTerm.clozes = newTerm.clozes.slice();
      if (trainSpeaking) {
        newTerm.clozes.forEach((c: Cloze) => {
          c.segment = c.segment.replace(/^speak:/, "train:");
        });
      } else {
        newTerm.clozes.forEach((c: Cloze) => {
          c.segment = c.segment.replace(/^train:/, "speak:");
        });
      }
    });
  ...
  flipSpeak$ = this.mcdTermActionHandler<EditTermFlipSpeak>(EditTermFlipSpeak,
    (action: EditTermFlipSpeak, newState: McdEditorTermState) => {
      var speakIt = newState.speakIt = !newState.speakIt;
      newState.trainSpeaking = newState.trainSpeaking && newState.speakIt;
      ...
      if (speakIt) {
        ...
        newState.editing.clozes = newState.editing.clozes.filter(c => !c.speakMatch());
      }
    });
  ...
  selectTextCell$ = this.mcdActionHandler<SelectTextCell>(SelectTextCell,
    (action: SelectTextCell, last: McdEditorState) => {
      ...
      if (last.termState.selectedRegion !=
        action.region ||
        last.termState.selectedRegionIdx >
        action.regionIdx) {
        return transformMcd(last)(state => {
          state.termState = transformTerm(state.termState)(term => {
      ...
      return transformMcd(last)((state: McdEditorState) => {
        ...
        term.clozes[0].schedule.intervalMinutes = 60 * 24;
        term.clozes[0].schedule.dueAtMinutes = Date.now() / 1000 / 60 + 60 * 2;
  ...
  answerCard = tap(this.interactions.interaction<number>())(interaction => {
    this.accumulator<number>(interaction.subject, (factor, last) => {
      ...
      return transformState(last)(next => {
        ...
        if (factor < 0) {
          scheduleUpdate.correction = true;
          factor = -factor;
        }
